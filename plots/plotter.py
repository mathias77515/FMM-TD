...
class PlotsMM:
    ...
    def plot_FMM(self, m_in, m_out, center, seenpix, nus, job_id, figsize=(10, 8), istk=1, nsig=6, fwhm=0):
        ...
        for i in range(self.params['QUBIC']['nrec']):
            ...
            if istk == 0:
                IN = C(m_in[i, :, istk])
                OUT = C(m_out[i, :, istk])
                sig_neg = 0#p.std(OUT[seenpix])
                sig_pos = np.std(m_out[-1, seenpix, istk])
            else:
                IN = C(m_in[i, :, istk])
                OUT = C(m_out[i, :, istk])
                sig = np.std(OUT[seenpix])
                sig_neg = 0#np.std(OUT[seenpix])
                sig_pos = np.std(m_out[-1, seenpix, istk])
            
            IN[~seenpix] = hp.UNSEEN
            OUT[~seenpix] = hp.UNSEEN
            
            #print(IN.shape)
            #print(OUT.shape)
            #print(OUT[seenpix])
            #print(np.std(OUT[seenpix]))
            hp.gnomview(IN, rot=center, reso=15, cmap='jet', min = - nsig * sig_neg, max = nsig * sig_pos, sub=(self.params['QUBIC']['nrec'], 3, k),
                        title=r'Input - $\nu$ = '+f'{nus[i]:.0f} GHz')
            hp.gnomview(OUT, rot=center, reso=15, cmap='jet', min = - nsig * sig_neg, max = nsig * sig_pos, sub=(self.params['QUBIC']['nrec'], 3, k+1),
                        title=r'Output - $\nu$ = '+f'{nus[i]:.0f} GHz')
            res = IN - OUT
            res[~seenpix] = hp.UNSEEN
            hp.gnomview(res, rot=center, reso=15, cmap='jet', min = - nsig * sig_pos, max = nsig * sig_pos, sub=(self.params['QUBIC']['nrec'], 3, k+2))
