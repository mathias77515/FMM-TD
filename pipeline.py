...
class PipelineFrequencyMapMaking:
    ...
    def __init__(self, comm, file):
        ...
        self.joint_tod = acq.JointAcquisitionFrequencyMapMakingTD(self.dict, self.params['QUBIC']['nsub'], self.params['QUBIC']['nsub'], H=None)
        self.joint_reco = acq.JointAcquisitionFrequencyMapMakingTD(self.dict, self.params['QUBIC']['nrec'], self.params['QUBIC']['nsub'], H=self.joint_tod.qubic.H)

        self.planck_acquisition143 = acq.PlanckAcquisition(143, self.joint_tod.qubic.scene)
        
        self.nus_Q = self._get_averaged_nus()
     

        ### Coverage map
        self.coverage = self.joint_reco.qubic.coverage.copy()
        ...
        self.invN = self.joint_reco.get_invntt_operator(mask=self.mask)
        ...
        qubic_noise = QubicNoiseTD(self.params['QUBIC']['npointings'], comm=comm, size=self.size, detector_nep=self.params['QUBIC']['detector_nep'])

        self.noiseq = qubic_noise.total_noise(self.params['QUBIC']['ndet'], 
                                       self.params['QUBIC']['npho150']).ravel()
    ...
    def _get_H(self):
        ...
        self.H = self.joint_reco.get_operator(fwhm=self.targets)
        #print(self.targets)
    ...
    def _get_averaged_nus(self):
        ...
        for i in range(self.params['QUBIC']['nrec']):
            nus_eff += [np.mean(self.joint_reco.qubic.allnus[i*self.fsub:(i+1)*self.fsub])]
    ...
    def _get_tod(self):
        ...
        for irec in range(int(self.params['QUBIC']['nrec'])):
            if self.params['QUBIC']['convolution']:
    ...
    def _pcg(self, d):
        ...
        if self.params['QUBIC']['nrec'] == 1:
            x0 = self.m_nu_in[0].copy()
        else:
            x0 = np.array([self.m_nu_in[0]]*self.params['QUBIC']['nrec'])
